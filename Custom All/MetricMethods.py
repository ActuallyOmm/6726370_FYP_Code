...
def lossPlot(train_losses, val_losses, title):

    # Reference for seeing if list contains element above certain value:
    # Resource: https://www.geeksforgeeks.org/check-if-element-exists-in-list-in-python/
    if any(x > 3 for x in train_losses) or any(x > 3 for x in val_losses) :
        plt.ylim(0,3)
    plt.plot(train_losses, label='Training Loss')
    plt.plot(val_losses, label='Validation Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Training and Validation Loss Curves '+ title)
    plt.legend()
    plt.show()

def accuracyPlot(train_accuracy, val_accuracy, title):
    plt.title(title)
    plt.plot(train_accuracy, label='Training Acc')
    plt.plot(val_accuracy, label='Validation Acc')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.title('Training and Validation Acc Curves '+ title)
    plt.legend()
    plt.show()
